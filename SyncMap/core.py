...
# %% auto 0
__all__ = ['SyncMap', 'SymmetricalSyncMap', 'LightSyncMap', 'VariableTracker', 'NodeSyncMap', 'd2p_exp', 'd2p_recip',
           'generated_adjm', 'visualize_adjm']
...
from .processor import (GraphProcessor, Readout,
                                        WorkingMemProcessor)
from .utility import (OverlapChunkTest1, compute_combi_dist,
...
class SymmetricalSyncMap:
    ...
    def adapt_chunking(self, input_state_vec, current_state_idx):
        ...
        dist_set2centroid_positive = distance.cdist(centroid_positive[None, :], self.syncmap, 'euclidean').T + 1e-12
        dist_set2centroid_negative = distance.cdist(centroid_negative[None, :], self.syncmap, 'euclidean').T + 1e-12
...
class LightSyncMap:
	...
	def __init__(self, input_size, dimensions, 
			adaptation_rate, use_tqdm = True, fix_seed = True):
...
class VariableTracker(object):
    ...
    def write(self, vec):
        ...
        '''
        if (self.last_vector is not None) or (not np.all(self.last_vector == vec)):  # change 'and' to 'or' at 2024-08-02
...
class NodeSyncMap(LightSyncMap):
    def __init__(self, input_size, dimensions, adaptation_rate, 
                ...
                std_factor = 2, fix_seed = True
                ):
        super().__init__(input_size, dimensions, adaptation_rate, use_tqdm = use_tqdm, fix_seed = fix_seed)
    ...
    def one_step_organize(self, vplus, vminus, current_state_idx):
        syncmap_previous = self.syncmap.copy()
        # syncmap_previous += np.random.normal(-1e-5, 1e-5, syncmap_previous.shape)  # comment without test
        center_plus  = self.get_center(vplus, syncmap_previous)
        center_minus = self.get_center(vminus, syncmap_previous)
        ...
        if self.normalization:
            new_syncmap = (new_syncmap - new_syncmap.mean()) / (self.std_factor * new_syncmap.std() + 1e-10) 
...
def d2p_exp(dists):
    '''
    args:
        dists(D, D): np.ndarray, the distance matrix
    '''
    return np.exp(-dists) - np.identity(dists.shape[0])
...
def d2p_recip(dists):
    ...
    args:
        dists(D, D): np.ndarray, the distance matrix
    '''
    return 1 / (dists + 1e-10) - 1 / 1e-10 * np.identity(dists.shape[0])


def generated_adjm(neuron_group, idx =-1, tau = 1, d2p = 'exp'):
    '''
    args:
        neuron_group: NodeSyncMap, the syncmap
        idx: int, the index of the syncmap
        d2p: str, the method to convert distance to probability
    return:
        np.ndarray, the generated adjacency matrix
    '''
    a0 = neuron_group.log[idx]
    adjm = distance.cdist(a0, a0, 'euclidean')
    if d2p == 'exp':
        adjm = d2p_exp(adjm/tau)
    elif d2p == 'recip':
        adjm = d2p_recip(adjm)

    normalized_adjm = adjm / adjm.sum(axis=1, keepdims=True)
    return normalized_adjm

def visualize_adjm(adjm):
    
    generated_graph = GraphProcessor()
    generated_graph.set_graph_from_adjacency_matrix(adjm)

    fig, axs = plt.subplots(1, 2, figsize = (12, 5))
    sns.heatmap(adjm, ax = axs[0], annot=True, fmt=".2f", vmin=0, vmax=0.25)
    generated_graph.visualize_graph(graph_layout = 4, with_probs = True)
    fig.tight_layout()
    return fig
